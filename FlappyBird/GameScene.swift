...
class GameScene: SKScene, SKPhysicsContactDelegate, AVAudioPlayerDelegate {
    ...
    //コイン関連
    var pickedItem = 0
    var pickedItemLabelNode: SKLabelNode!
    ...
    func setupGround() {
        ...
        //左にスクロール元の位置左にスクロールと無限に繰り返すアクション
    ...
    func setupWall() {
        ...
        let createWallAnimation = SKAction.run({
            ...
            wall.addChild(scoreNode)    //ここまでスコアnode
    ...
    func setupItem() {
        ...
        wallTex.filteringMode = SKTextureFilteringMode.linear
        
        //移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width + wallTex.size().width)
        ...
        let createItemAnimation = SKAction.run({
            // 壁関連のノードを乗せるノードを作成
            let item = SKNode()
            item.position = CGPoint(x: self.frame.size.width + wallTex.size().width / 2, y: 0.0)
            ...
            // Y軸の下限にランダムな値を足して、下の壁のY座標を決定
            let randomPoint = CGFloat(UInt32(center_y) + random_y)
            
            // コインを作成
            let itemSprite = SKShapeNode(circleOfRadius: 10.0)
            itemSprite.position = CGPoint(x: 0.0, y: randomPoint)
            itemSprite.fillColor = UIColor.yellow
            itemSprite.strokeColor = UIColor.yellow
            item.addChild(itemSprite)
            
            //スプライトに物理演算を設定する
            itemSprite.physicsBody = SKPhysicsBody(circleOfRadius: itemSprite.frame.size.width / 2.0)
    ...
    func setupScoreLabel() {
        ...
        scoreLabelNode.fontColor = UIColor.darkGray
        ...
        bestScoreLabelNode.fontColor = UIColor.darkGray
        ...
        pickedItem = 0
        pickedItemLabelNode = SKLabelNode()
        pickedItemLabelNode.fontColor = UIColor.darkGray
        pickedItemLabelNode.position = CGPoint(x: 10, y: self.frame.size.height - 90)
        pickedItemLabelNode.zPosition = 100 // 一番手前に表示する
        pickedItemLabelNode.horizontalAlignmentMode = SKLabelHorizontalAlignmentMode.left
        pickedItemLabelNode.text = "Item:\(pickedItem)"
        self.addChild(pickedItemLabelNode)
    }
